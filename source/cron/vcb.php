<?php
    ...
    $token = $SIEUTHICODE->site('token_vcb');
    $result = curl_get("https://api.sieuthicode.net/historyapivcbv2/$token");
                    ...
                    if ($create)
                    {
                        ...
                        if($isCheckMoney)
                        {
                            ...
                            $SIEUTHICODE->insert("dongtien", array(
                                ...
                                'thoigian' => gettime(),
                                'noidung' => 'Nạp tiền tự động ngân hàng (VCB | '.$tid.')',
                                'username' => $row['username']
                            ));
                        }
                    }
