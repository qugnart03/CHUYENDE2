...
$listOrder = $SIEUTHICODE->get_list(" SELECT * FROM `history_buy` WHERE $where AND `username_post` = '".$getUser['username']."' ORDER BY id DESC LIMIT $from,$sotin1trang ");
$dataSumary = $SIEUTHICODE->get_list("SELECT * FROM `history_buy` WHERE $where AND `username_post` = '".$getUser['username']."'");
$totalTransactions = 0;
$totalAmount = 0;
$totalAmountReal = 0;
                        ...
                        <div class="card-body">
                                    ...
                                    <form action="" name="formSearch" method="GET">
                                        <div class="row">
                                            ...
                                            <div class="input-group mb-3 col-12 col-sm-6 col-lg-3">
                                                ...
                                                <select id="type_category" class="form-control datatable-input select2bs4"
                                                    name="type_category">
                            ...
                            <div class="row mb-2">
                                <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                                    Tổng số nick đã bán: <b id="total_record"><?=format_cash($totalTransactions)?></b> - Doanh thu (Giá gốc): <b id="total_price"><?=format_cash($totalAmount)?></b> - Doanh thu của CTV (Được hưởng): <b id="total_price"><?=format_cash($totalAmountReal)?></b>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-7">
                                    <?php
                                $total = $SIEUTHICODE->num_rows(" SELECT * FROM `history_buy` WHERE $where AND `username_post` = '".$getUser['username']."' ORDER BY id DESC ");
                                if ($total > $sotin1trang){echo '<center>' . pagination(BASE_URL("Ctv/Account-Sold?idacc=$idacc&username=$username&type_category=$type_category&create_date=$create_date&"), $from, $total, $sotin1trang) . '</center>';}?>
                                </div>
                            </div>
                        </div>
                    </div>
