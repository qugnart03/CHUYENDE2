...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Bán vàng NRO - Thông tin bot nạp</h1>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <section class="col-lg-6 text-right">
                    <div class="mb-3">
                        <a class="btn btn-primary btn-icon-left m-b-10" href="<?=BASE_URL('Admin/toolgame/nrocoinlau-usernap/create');?>"
                ...
                <div class="col-md-12">
                    <div class="card card-outline card-primary">
                        <div class="card-header">
                            <h3 class="card-title">Bán vàng NRO - Thông tin bot nạp</h3>
                        ...
                        <div class="card-body">
                            <div class="table-responsive">
                                <table id="datatable" class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody>
                                        <?php foreach ($SIEUTHICODE->get_list("SELECT * FROM `coin_nro_lau` ORDER BY `server_nro` ASC") as $info):?>
                                        <tr>
                                            <td><?=$info['id']?></td>
                                            <td><?=$info['server_nro'];?></td>
                                            <td><?=format_cash($info['factor'])?></td>
                                            <td><?=format_cash($info['min_value'])?></td>
                                            <td><?=format_cash($info['max_value'])?></td>
                                            <td><?=display_status_product($info['status'])?></td>
                                            <td>
                                                <a class="btn btn-info btn-sm"
                                                    href="/Admin/toolgame/nrocoinlau-usernap/edit/<?=$info['id']?>"><i
                                                        class="fa fa-pen"></i></a>
...
function delete_id(id) {
    ...
    if (confirm(text) == true) {
        $.ajax({
            url: '/Model/Admin/BotNapCoinLau/Delete',
            type: 'POST',
            dataType: 'JSON',
            data: {
                id
