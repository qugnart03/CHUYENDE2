...
$listOrder = $SIEUTHICODE->get_list(" SELECT * FROM `history_coin_nro_lau` WHERE $where ORDER BY id DESC LIMIT $from,$sotin1trang ");
$dataSumary = $SIEUTHICODE->get_list("SELECT * FROM `history_coin_nro_lau` WHERE $where");
$totalGoldNumber = 0;
$totalGoldBar = 0;
$totalTransactions = 0;
$totalAmount = 0;
                    ...
                    <div class="card card-outline card-primary">
                        ...
                        <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-4 mb-2">
                                                ...
                                                <a class="btn btn-danger"
                                                    href="/Admin/toolgame/nrocoinlau-logtransaction"><i
                                                        class="fa fa-trash"></i>
                                                    Reset
                                                </a>
                                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                                    Số thỏi vàng bán: <b><?=format_cash($totalGoldNumber)?></b>
                                </div>
                                <div class="col-lg-12 m--margin-bottom-10-tablet-and-mobile" style="font-size: 14px ">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td><button class="btn btn-info"
                                                    onclick="show(<?=$row['id'];?>,<?=$row['status'];?>)"><i
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-md-7">
                                    <?php
                                $total = $SIEUTHICODE->num_rows(" SELECT * FROM `history_coin_nro_lau` WHERE $where ORDER BY id DESC ");
                                if ($total > $sotin1trang){echo '<center>' . pagination(BASE_URL("Admin/toolgame/nrocoinlau-logtransaction?transid=$transid&username=$username&server=$server&player=$player&create_date=$create_date&"), $from, $total, $sotin1trang) . '</center>';}?>
                                </div>
                            </div>
                        </div>
                    </div>
...
function change(){
    $('#change').html('Đang xử lý...').prop('disabled',
        true);
    $.ajax({
        url: "/Model/Admin/ChangeHistoryNroLau",
        method: "POST",
        dataType:"JSON",
        data: {
            id: $("#iddiamond").val(),
        ...
        success: function(response) {
            if (response.status == 'success') {
                ...
                setTimeout(function () {
                            window.location = '/Admin/toolgame/nrocoinlau-logtransaction';
                        }, 1000);
            }else {
                Toast(response.status,response.msg)
            }
