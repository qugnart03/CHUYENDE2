...
{
    $row = $SIEUTHICODE->get_row(" SELECT * FROM `gem_api_nro` WHERE `id` = '".Anti_xss($_GET['id'])."'  ");
    if(!$row)
    {
        new Redirect('/');
    }
...
if (isset($_POST['Update']) && $getUser['level'] == 'admin') {
    ...
    $isInsert = $SIEUTHICODE->update("gem_api_nro", [
    ...
    if ($isInsert) {
        insert_log($getUser['id'],"Chỉnh sửa thông tin bán ngọc (ID ".$row['id'].").");
...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Bán ngọc NRO - Thông tin bot nạp - Cập nhật</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="<?=BASE_URL('Admin/Home');?>">Dashboard</a></li>
                        <li class="breadcrumb-item active">Bán ngọc NRO - Thông tin bot nạp - Cập nhật</li>
                    </ol>
                </div>
            </div>
        </div>
