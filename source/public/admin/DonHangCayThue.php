<?php
    ...
    $title = 'DANH SÁCH ĐƠN HÀNG | '.$SIEUTHICODE->site('tenweb');
...
if(isset($_POST['Save']) && $getUser['level'] == 'admin' )
{
    ...
    $row = $SIEUTHICODE->get_row("SELECT * FROM `orders_caythue` WHERE `id` = '$id' ");
    if(!$row)
    {
    ...
    {
        // refund
        $isMoney = $SIEUTHICODE->cong("users", "money", $row['money'], " `username` = '".$row['username']."' ");
        if($isMoney)
        {
            $getUser = $SIEUTHICODE->get_row("SELECT * FROM `users` WHERE `username` = '".$row['username']."' ");
            /* GHI LOG DÒNG TIỀN */
    ...
    $SIEUTHICODE->update("orders_caythue", array(
        'status' => $status
                            ...
                            <div class="table-responsive">
                                <table id="datatable1" class="table table-bordered table-striped table-hover">
                                    ...
                                    <tbody>
                                        <?php
                                    $i = 0;
                                    foreach($SIEUTHICODE->get_list(" SELECT * FROM `orders_caythue` ORDER BY id DESC ") as $row){
                                    ?>
                                        <tr>
                                            <td><?=$i++;?></td>
                                            <td><?=$row['dichvu'];?></td>
                                            <td><?=decodecryptData($row['tk']);?>|<?=decodecryptData($row['mk']);?>|<?=$row['maychu'];?></td>
                                            <td><textarea class="form-control" readonly><?=$row['ghichu'];?></textarea>
                                            </td>
                                            ...
                                            <td>
                                                <button class="btn btn-primary" onclick="editOrder(<?=$row['id'];?>)"><i
                                                        class="fas fa-edit"></i>
