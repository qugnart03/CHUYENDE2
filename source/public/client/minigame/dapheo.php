...
.v-image-1 {
    ...
    opacity: 1;
    margin: auto;
    background: url('<?=BASE_URL($detail['vongquay'])?>') no-repeat center center;
            ...
            <div class="col-span-12 sm:col-span-8 md:col-span-8 lg:col-span-8 xl:col-span-8">
                <div class="py-6 bg-white rounded"
                    ...
                    <div class="w-full relative">
                        <div class="mt-8 max-w-xl mx-auto px-3 md:px-5">
                            <div class="flex justify-center w-full">
                                <div class="relative item item-left">
                                    <div class="v-main relative item item-left">
                                        <section class="v-rotation">
                                            <div class="bgnen"></div>
                                            <input type="hidden" value="<?=BASE_URL($detail['open'])?>" id="hdImageLD">
                                            <input type="hidden" value="<?=BASE_URL($detail['close'])?>"
                                                id="beforePlay">
                                            <img class="v-image-1 lazyLoad" data-src="<?=BASE_URL($detail['close'])?>" src="<?=BASE_URL('');?>/assets/img/index.svg">
                                        </section>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="my-2 flex justify-center items-center"><button type="button" id="try"
...
$(document).ready(function(e) {
    ...
    function loop() {
        ...
        if (gift_revice.length > 0) {
            $html = "";
            if (typeRoll == "play") {
                $html += "<p>Kết quả quay <span class='text-red-600'>(Quay Thật):</span> Quay " + gift_revice
                    .length + " lần - giá " + gift_price + "đ</p>";
                $html += "<div><b>Mua X" + gift_revice.length +
                    ": Tổng Trúng</br><p class='text-md uppercase inline-block rounded border-2 border-red-500 px-2 text-red-500 mr-1'>" +
                    gift_total + " kc</p></b></div>";
                $html += "<div class='h-2'></div>";
                for ($i = 0; $i < gift_revice.length; $i++) {
                    $html += "<p class='text-md'>- Quay lần " + ($i + 1) + ": " + gift_revice[$i]["title"];
                }
            } else {
                $html += "<p>Kết quả quay <span class='text-red-600'>(Quay Thử):</span> Quay " + gift_revice
                    .length + " lần - giá " + gift_price + "đ</p>";
                $html += "<div><b>Mua X" + gift_revice.length +
                    ": Tổng Trúng</br><p class='text-md uppercase inline-block rounded border-2 border-red-500 px-2 text-red-500 mr-1'>" +
                    gift_total + " kc</p></b></div>";
