    ...
    <div class="mb-4 py-4 md:p-4 bg-box-dark">
        ...
        <div class="tw-mb-2 mb-2 p-2">
            <div class="text-xl bg-blue-100 text-gray-900 p-2 px-3 rounded mb-4 panel-body hidetext">
                <?=$SIEUTHICODE->site('notice_banvang')?>
            </div>
        ...
        <div class="mt-4">
            <div class="grid grid-cols-12 gap-4 md:p-4">
                <div class="col-span-12 sm:col-span-12 md:col-span-2 lg:col-span-2 xl:col-span-2 lg:px-0 px-2">
                    <img class="w-full object-fill object-center rounded cursor-pointer"
                        src="<?=BASE_URL($SIEUTHICODE->site('logo_banvang'))?>">
                    <p class="text-white mt-2"><i class="fa fa-calendar" aria-hidden="true"></i> BÁN VÀNG TỰ ĐỘNG</p>
                </div>
                        ...
                        <div class="v-infomations mb-10">
                            <div class="px-5">
                                <span class="mb-2 block">
                                    ...
                                    <div class="flex items-center relative">
                                        <select id="server" onchange="selectData()"
                                            ...
                                            <?php foreach ($SIEUTHICODE->get_list("SELECT * FROM `coin_nro` WHERE `status` = '1' ORDER BY `server_nro` ASC ") as $group) {?>
                                ...
                                <span class="mb-2 block">
                                    ...
                                    <div class="flex items-center relative"><input placeholder="Nhân vật"
                                            id="player"
                                ...
                                <span class="mb-2 block">
                                    <label class="text-white">Số vàng nhận được:</label>
                                    <div class="flex items-center relative"><input readonly
                                            placeholder="Số vàng nhận được" id="actually"
                                            class="border border-gray-500 rounded bg-white text-gray-800 appearance-none w-full py-2 px-3 leading-tight focus:outline-none">
                                    </div>
                                </span>
                                <div class="mt-4 text-center"><button type="button"  onclick="Pay()" id="Submit"
                ...
                <div class="col-span-12 sm:col-span-12 md:col-span-6 lg:col-span-6 xl:col-span-6 px-2 md:px-0">
                    <div class="v-table-content select-text">
                        <div class="py-2 max-w-400">
                                ...
                                <tbody class="text-sm font-semibold">
                                    ...
                                    <tr>
                                        <td class="text-sm text-white text-left px-1 py-1 border-b">
                                            <?=$info['server_nro']?></td>
                                        <td class="text-sm text-white text-left px-1 py-1 border-b"><?=$info['player']?>
                                        </td>
                                        <td class="text-sm text-white text-left px-1 py-1 border-b">
                                            <?=$info['location']?></td>
                                        <td class="text-sm text-white text-left px-1 py-1 border-b"><?=$info['zone']?>
                                        </td>
                                        <td class="text-sm text-white text-left px-1 py-1 border-b">
                                            <?=status_bot_nro($info['status'])?></td>
                                    </tr>
                                    <?php endforeach;?>
                                </tbody>
                            </table>
                        ...
                        <div class="py-2 overflow-x-auto scrolling-touch max-w-400">
                            <table id="datatable" class="table-auto w-full scrolling-touch min-w-850">
                                ...
                                <tbody class="text-sm font-semibold">
                                    ...
                                    <?php $i = 0; foreach($SIEUTHICODE->get_list(" SELECT * FROM `history_coin_nro` WHERE `user_id` = '".$getUser['id']."' ORDER BY id DESC ") as $info){ 
                                    ?>
                                    <tr>
                                        ...
                                        <td class="px-2 py-2 text-xs border-b">
                                            <div class="block mt-2 pt-2 font-semibold leading-5">
                                                ...
                                                <p
                                                    ...
                                                    Nhận:
                                                    <b> <?=format_cash($info['gold_number'])?> =
                                                        <?=format_cash($info['gold_bar'])?> Thỏi vàng +
                                                        <?=format_cash($info['remaining'])?> Vàng</b>
                                                </p>
                                            </div>
                                        </td>
                                        <td class="px-2 py-1 md:py-2 text-xs border-b">
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.status == 'success') {
                ...
                setTimeout(function() {
                    window.location = '/history/coin-nro';
                }, 1000);
            } else {
                Toast(response.status, response.msg)
            }
...
function selectData() {
    ...
    function calculateAmount() {
        ...
        const money = parseInt(moneyInput.value.replace(/\D/g,
            ''));
        const factorInput = document.getElementById('factor');
        const factor = parseInt(factorInput.value.replace(/\D/g,
            ''));

        if (!isNaN(money) && !isNaN(factor)) {
            const totalAmount = factor * money;
            resultOutput.value = formatNumberWithCommas(totalAmount);
        }
    }
