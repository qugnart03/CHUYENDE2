<?php
    ...
    $title = 'BIẾN ĐỘNG SỐ DƯ | '.$SIEUTHICODE->site('tenweb');
                    ...
                    <div class="py-2 overflow-x-auto scrolling-touch max-w-400">
                        <table id="datatable" class="table-auto w-full scrolling-touch min-w-850">
                            ...
                            <tbody class="text-sm font-semibold">
                                <?php $i = 0; foreach($SIEUTHICODE->get_list(" SELECT * FROM `dongtien` WHERE `username` = '".$getUser['username']."' ORDER BY id DESC ") as $row){ ?>
                                <tr>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><?=$i++;?></td>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><?=format_cash($row['sotientruoc']);?></td>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><?=format_cash($row['sotienthaydoi']);?></td>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><?=format_cash($row['sotiensau']);?></td>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><span class="badge badge-dark"><?=$row['thoigian'];?></span></td>
                                    <td class="text-sm text-dark text-left px-1 py-1 border-b"><?=$row['noidung'];?></td>
                                </tr>
                                <?php }?>
                            </tbody>
                        </table>
