...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    if(isset($_GET['player'])){
        ...
        if(!$SIEUTHICODE->get_row("SELECT * FROM `history_coin_nro` WHERE `status` = 0 AND `player`= '$player'")){
            die(json_encode(array('status'=>false,'msg'=>'Nhân vật này không có đơn hàng')));
        }
        $listData = array();
        foreach($SIEUTHICODE->get_list("SELECT * FROM `history_coin_nro` WHERE `status` = 0 AND `player`= '$player'") as $row){
            $listData[] = $row;
        }
