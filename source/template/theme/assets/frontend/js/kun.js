...
for (const el of closeEls) {
    el.addEventListener("click", function() {
        ...
        this.parentElement.parentElement.parentElement.parentElement.parentElement.classList.add("out");
    });
}
...
function Tab(id){
  ...
  for (i = 0; i < tablinks.length; i++) {
    ...
    tablinks[i].className = tablinks[i].className.replace("bg-trueGray-900", "bg-trueGray-800");
  }
  document.getElementById(id).style.display = "block";
  event.currentTarget.className += " active";
  event.currentTarget.className = event.currentTarget.className.replace("bg-trueGray-800", "bg-trueGray-900");
}
