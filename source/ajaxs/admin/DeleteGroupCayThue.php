...
if (isset($_POST['id'])) {
    ...
    if (!$row = $SIEUTHICODE->get_row("SELECT * FROM `groups_caythue` WHERE `id` = '$id' ")) {
        die(JsonMsg('error', 'ID nhãn dán không tồn tại trong hệ thống!'));
    }
    $isRemove = $SIEUTHICODE->remove("groups_caythue", " `id` = '$id' ");
    if ($isRemove) {
        $SIEUTHICODE->insert("logs", [
            ...
            'create_date'    => gettime(),
            'action'        => 'Xoá nhóm cày thuê ('.$row['title'].' ID '.$row['id'].')'
        ]);
        die(JsonMsg('success', 'Xoá nhóm cày thuê thành công'));
    }
} else {
