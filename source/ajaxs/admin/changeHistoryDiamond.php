...
if (isset($_POST['id']) && isset($_POST['status'])) {
    ...
    if (!$row = $SIEUTHICODE->get_row("SELECT * FROM `withdraw` WHERE `id` = '$id' ")) {
        die(JsonMsg('error', 'ID lịch sử không tồn tại trong hệ thống!'));
    }

    $isUpdate = $SIEUTHICODE->update("withdraw", [
        'status' => $status,
        'update_date' => time(),
    ], " `id` = '" . $row['id'] . "' ");
    ...
    if ($isUpdate) {
        $SIEUTHICODE->insert("logs", [
            ...
            'create_date'    => gettime(),
            'action'        => 'Chỉnh sửa trạng thái lịch sử rút diamond (ID '.$row['id'].')'
        ]);
