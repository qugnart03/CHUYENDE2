...
if (isset($_POST['id'])) {
    ...
    if (!$row = $SIEUTHICODE->get_row("SELECT * FROM `tag` WHERE `id` = '$id' ")) {
        die(JsonMsg('error', 'ID nhãn dán không tồn tại trong hệ thống!'));
    }
    $isRemove = $SIEUTHICODE->remove("tag", " `id` = '$id' ");
    if ($isRemove) {
        $SIEUTHICODE->insert("logs", [
            ...
            'create_date'    => gettime(),
            'action'        => 'Xoá nhãn dán ('.$row['name'].' ID '.$row['id'].')'
        ]);
        die(JsonMsg('success', 'Xóa nhãn dán thành công'));
    }
} else {
