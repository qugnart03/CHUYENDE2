...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif ($SIEUTHICODE->num_rows("SELECT * FROM `nrocoin_info_bot` WHERE `id` = '{$id}'") < 1) {
        echo JsonMsg('error', 'Không tìm thấy bot bạn cần xóa');
    } else {
        $SIEUTHICODE->query("DELETE FROM `nrocoin_info_bot` WHERE `id` = '{$id}'");
        insert_log($getUser['id'], 'Xóa bot bán vàng #' . $id . '');
        echo JsonMsg('success', 'Xóa thành công bot bán vàng #' . $id . '');
    }
} else {
